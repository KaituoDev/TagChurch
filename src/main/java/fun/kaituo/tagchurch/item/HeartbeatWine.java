...
public class HeartbeatWine extends ActiveItem {
    ...
    public boolean use(Player p) {
        ...
        if (p.getHealth() * 2 < p.getMaxHealth()) {
            p.addPotionEffect(new PotionEffect(PotionEffectType.GLOWING, (int) (p.getMaxHealth() * 10), 0, false, false));
            p.setHealth(p.getHealth() + p.getMaxHealth() / 2);
        } else {
            p.addPotionEffect(new PotionEffect(PotionEffectType.GLOWING, (int) ((p.getMaxHealth() - p.getHealth()) * 20), 0, false, false));
