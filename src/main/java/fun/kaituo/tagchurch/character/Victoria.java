...
public class Victoria extends Human {
    ...
    public Victoria(Player p) {
        ...
        maidItem = TagChurch.inst().getItem("VictoriaMaid");
        succubusItem = TagChurch.inst().getItem("VictoriaSuccubus");
        taskIds.add(Bukkit.getScheduler().scheduleSyncRepeatingTask(TagChurch.inst(), this::regenerate, 50, 50));
    }
    ...
    private void regenerate() {
        if (!isMaid) {
            for (Entity e : player.getNearbyEntities(radius, radius, radius)) {
                ...
                if (TagChurch.inst().getPlayers().contains(p) && !p.getGameMode().equals(GameMode.SPECTATOR)) {
